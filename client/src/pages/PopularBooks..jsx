        ...
        >
          {popularBooks.map((book, index) => (
            <div key={index} className="py-6 px-4">
              <div className=" bg-white rounded-lg shadow-xl flex flex-col items-center justify-center p-1 hover:scale-110 transition-transform duration-30 ">
                <img src={book.image} alt={book.title} className=" h-[42vh] w-1/5 object-fit rounded-t-md  " />
                <div className="p-2 text-center">
                  <p className="text-black font-abril">{book.title}</p>
                </div>
              </div>
            </div>
          ))}
